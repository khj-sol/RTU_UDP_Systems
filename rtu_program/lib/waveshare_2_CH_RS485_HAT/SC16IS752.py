...
# -*- coding:utf-8 -*-
"""
SC16IS752 Dual UART with SPI Interface
Version: 1.0.9 - Multi-Platform Support (Pi Zero 2W / Pi 5 Dual Compatible)

Changes in 1.0.9:
  - Updated for Pi Zero 2W and Pi 5 dual compatibility
...
Changes in 1.0.7:
  - Compatible with RS232 RS485 CAN HAT+ (Auto Direction Control)
  - IRQ pin changed to GPIO22 for new HAT
"""
import lgpio
import logging
...
class SC16IS752(object):
    ...
    def __init__(self, cfg, Channel=CHANNEL_1):
        """
        Initialize SC16IS752 with config instance
        Args:
            cfg: config instance (singleton)
            Channel: CHANNEL_1 or CHANNEL_2
        """
        self.com = cfg
        self.IRQ_PIN = cfg.IRQ_PIN
        self._gpio_handle = cfg._h
    ...
    def WR_REG(self, Reg, Value):
        if(type(Value) != int):
            for i in range(0, len(Value)):
                Value[i] = ord(Value[i])
            Data_t = self.com.SPI_transmission_nByte([Reg]+ Value)
        else:
            Data_t = self.com.SPI_transmission_nByte([Reg,Value])
    ...
    def UART_Read(self, Len):
        ...
        buf = ''
        while(1):
            if(lgpio.gpio_read(self._gpio_handle, self.IRQ_PIN) == 0):
