...
class SolarizePvStatusConverter:
...
StatusConverter = SolarizePvStatusConverter
...
def generate_iv_voltage_data(voc, v_min=200.0, data_points=64):
...
def generate_iv_current_data(isc, voc, v_min=200.0, data_points=64):
    """Generate IV scan current array (U16, 0.01A units) 단일 다이오드 모델.

    I(V) = Isc * (1 - exp((V - Voc) / (n * Vt * Ns)))
    Ns = Voc/50 (직렬 셀 수 추정), n=1.3, Vt=0.026V(25C)
    """
    import math
    v_start = max(v_min, 200.0)
    step = (voc - v_start) / max(data_points - 1, 1)
...
DATA_PARSER = {
    'mode                ': 'INVERTER_MODE',
    'r_voltage           ': 'DEA_L1_VOLTAGE (0x03EE~0x03EF); L1_VOLTAGE (0x0026)',
    ...
    'frequency           ': 'DEA_FREQUENCY (0x03FA~0x03FB); FREQUENCY (0x000F)',
    'ac_power            ': 'DEA_TOTAL_ACTIVE_POWER (0x03F4~0x03F5); AC_POWER (0x1037)',
    'cumulative_energy   ': 'CUMULATIVE_ENERGY (0x1306)',
