...
def _is_register_table_page(page: dict) -> bool:
    """레지스터 테이블이 포함된 페이지인지 판단.

    조건: 주소 패턴(0x 또는 4~5자리 decimal)이 3개 이상이거나
    테이블 헤더에 address/addr 키워드가 있는 페이지.
    """
    text = page.get('text', '')
    tables = page.get('tables', [])

    hex_count = len(re.findall(r'\b0x[0-9A-Fa-f]{4}\b', text))
    dec_count = len(re.findall(r'\b\d{4,5}\b', text))
    if hex_count >= 3 or dec_count >= 5:
        return True

    addr_kw = {'address', 'addr', '주소', 'offset', 'register'}
    for tbl in tables:
        if not tbl:
            continue
        header = [str(c).lower().strip() for c in tbl[0] if c]
        if any(any(k in h for k in addr_kw) for h in header):
            return True

    return False


def _classify_pass1_with_rules(candidates: List[Dict], log=None) -> List[Dict]:
    """Pass1 AI 후보를 rule-based로 category/h01_field 배정 (Pass2 AI 대체)."""
    _CTRL_KW = ['on_off', 'power_limit', 'power_factor_set', 'reactive_power', 'power_derating',
                'operation_mode', 'active_power_limit', 'inverter_control']
    _STATUS_KW = ['mode', 'status', 'state', 'running']
    _ALARM_KW = ['error', 'fault', 'alarm', 'warning', 'protection']
    _INFO_KW = ['model', 'serial', 'firmware', 'version', 'nominal', 'rated', 'mppt_count',
                'device_type', 'manufacturer', 'rated_power', 'max_power']

    _H01_MAP = [
        (['l1_voltage', 'phase_a_voltage', 'grid_voltage_a', 'ua_voltage', 'r_voltage',
          'ac_voltage', 'grid_voltage'], 'ac_voltage'),
        (['l1_current', 'phase_a_current', 'grid_current_a', 'ia_current', 'r_current',
          'ac_current', 'grid_current'], 'ac_current'),
        (['active_power', 'ac_power', 'output_power', 'grid_power'], 'ac_power'),
        (['frequency', 'grid_freq', 'ac_frequency'], 'frequency'),
        (['power_factor', 'cos_phi'], 'power_factor'),
        (['reactive_power'], 'reactive_power'),
        (['daily_energy', 'today_energy', 'daily_yield'], 'daily_energy'),
        (['cumulative_energy', 'total_energy', 'lifetime_energy', 'accumulated_energy'], 'cumulative_energy'),
        (['inner_temp', 'heatsink_temp', 'module_temp', 'temperature'], 'inner_temp'),
        (['inverter_mode', 'running_status', 'inverter_status', 'operating_state'], 'inverter_status'),
        (['error_code', 'fault_code', 'alarm_code'], 'alarm'),
        (['device_model', 'inverter_model'], 'device_model'),
        (['device_serial', 'serial_number'], 'device_serial'),
    ]

    result = []
    seen_addr: Dict[str, bool] = {}
    for c in candidates:
        ...
        name_lower = c.get('raw_name', c.get('name', '')).lower()
        name_norm = re.sub(r'[^a-z0-9]', '_', name_lower)

        cat = c.get('category', '')
        if not cat:
            if any(k in name_norm for k in _CTRL_KW):
                cat = 'CONTROL'
            elif any(k in name_norm for k in _ALARM_KW):
                cat = 'ALARM'
            elif any(k in name_norm for k in _STATUS_KW):
                cat = 'STATUS'
            elif any(k in name_norm for k in _INFO_KW):
                cat = 'DEVICE_INFO'
            else:
                cat = 'MONITORING'

        h01 = c.get('h01_field') or None
        if not h01:
            for patterns, field in _H01_MAP:
                if any(p in name_norm for p in patterns):
                    h01 = field
                    break
        ...
        if not out.get('name'):
            raw = c.get('raw_name', '')
            out['name'] = re.sub(r'[^A-Z0-9_]', '_',
                                  raw.upper().replace(' ', '_'))[:40] if raw else f'REG_{addr}'
        result.append(out)

    if log:
        log(f'[RuleClassify] {len(result)}개 레지스터 분류 완료 (Pass2 대체)')
    return result
...
def _run_hybrid_ai_extraction(pdf_path: str, ai_settings: dict,
    ...
    파이프라인:
      1. PDF 텍스트/표 추출 (PyMuPDF, 기존 로직)
      2. 규칙 기반 regex 후보 추출
      3. Phi: JSON 포맷팅 + Critic 검증
      4. 희박 페이지(sparse_threshold 미만) 감지
      5. Qwen3-VL: 희박 페이지 이미지 직접 파싱
      6. 병합 (주소 기준 중복 제거, Qwen 결과 우선)
      7. Rule-based 분류

    폴백:
      - Phi 실패 → regex 결과 그대로
      - Qwen 실패 → 희박 페이지 건너뜀
      - 둘 다 실패 → rules-only (V3 동작과 동일)
    """
    def log(msg, level='info'):
        if progress:
            progress(msg, level)
    ...
    qwen_device = ai_settings.get('qwen_device', 'auto')
    qwen_wsl_url = ai_settings.get('qwen_wsl_url', '')
    image_dpi = int(ai_settings.get('image_dpi', 200))
    sparse_threshold = int(ai_settings.get('sparse_threshold', 3))

    # ── 1. PDF 텍스트/표 추출 ──────────────────────────────────────────────
    log('[Hybrid] PDF 텍스트 추출 중...')
    all_pages = extract_pdf_text_and_tables(pdf_path, log=log)
    log(f'[Hybrid] {len(all_pages)}페이지 추출 완료')

    # 레지스터 테이블 페이지 필터
    page_idx_map = {i: p for i, p in enumerate(all_pages) if _is_register_table_page(p)}
    if not page_idx_map:
        log('[Hybrid] 레지스터 테이블 페이지 감지 실패 → 전체 페이지 사용', 'warn')
        page_idx_map = {i: p for i, p in enumerate(all_pages)}
    else:
        log(f'[Hybrid] 레지스터 테이블 페이지 {len(page_idx_map)}/{len(all_pages)}페이지 필터링')

    # ── 2. 규칙 기반 regex 후보 추출 + 페이지별 밀도 계산 ────────────────
    log('[Hybrid] 규칙 기반 후보 추출 중...')
    regex_candidates: List[Dict] = []
    page_counts: List[tuple] = []  # (page_idx, reg_count)
    for idx, page in page_idx_map.items():
        page_regs = extract_registers_from_tables(page.get('tables', []))
    ...
    if ai_mode in ('phi_only', 'full') and regex_candidates:
        try:
            ...
            if phi:
                ...
                log('[Phi] Critic 검증 중...')
                phi_candidates, phi_removed = phi.critic_validate(phi_candidates)
                if phi_removed:
                    log(f'[Phi] {len(phi_removed)}개 제거: {phi_removed[:3]}')
                log(f'[Phi] 완료: {len(phi_candidates)}개 통과')
            else:
                log('[Phi] 모델 미로드 → regex 결과 사용', 'warn')
        except Exception as e:
            log(f'[Phi] 오류: {e} → regex 결과 사용', 'warn')
            phi_candidates = regex_candidates

    # 주소 기준 중복 제거 (Phi 결과를 베이스로)
    merged: Dict[str, Dict] = {}
    for c in phi_candidates:
        addr = str(c.get('address', '')).strip().lower()
        if addr:
            merged[addr] = c
        ...
        if sparse_indices:
            ...
            try:
                ...
                if qwen:
                    ...
                    log(f'[QwenVL] {len(qwen_regs)}개 추출 → 병합 (Qwen 결과 우선)')
                    for r in qwen_regs:
                        addr = str(r.get('address', '')).strip().lower()
                        if addr:
                            merged[addr] = r  # Qwen 결과가 regex/Phi를 덮어씀
                else:
                    log('[QwenVL] 모델 미로드 → 희박 페이지 건너뜀', 'warn')
            except Exception as e:
                log(f'[QwenVL] 오류: {e} → 희박 페이지 건너뜀', 'warn')
        else:
            log('[Hybrid] 모든 페이지 충분한 regex 후보 → Qwen 건너뜀')

    # ── 6. 최종 후보 목록 ─────────────────────────────────────────────────
    all_candidates = list(merged.values())
    log(f'[Hybrid] 병합 후보 {len(all_candidates)}개')

    if not all_candidates:
        log('[Hybrid] 추출 결과 없음 → 규칙 기반으로 전환', 'warn')
        return []
...
# ── 구 Ollama/Gemma 함수는 V4에서 _run_hybrid_ai_extraction으로 대체됨 ─────────
...
def run_stage1(
    ...
    if not _ai_mode_active or not registers:
        if _ai_mode_active and not registers:
            ...
            if ext == '.pdf':
                pages = extract_pdf_text_and_tables(input_path, log=log)
                # 섹션 구분 없이 모든 테이블 수집 (FC 태깅 없음)
                all_tables = []
                fc_list = []
                for _p in pages:
                    for _tab in _p['tables']:
                        all_tables.append(_tab)
                        fc_list.append('')
        log('레지스터 테이블 파싱...')
