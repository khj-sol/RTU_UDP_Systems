...
def _run_ai_pdf_extraction(pdf_path: str, ai_settings: dict,
    ...
    """Gemma (Ollama) 레지스터 추출.

    A: 레지스터 테이블 페이지만 필터링 → Gemma 입력량 대폭 감소
    B: Pass2 제거 → rule-based 분류로 대체
    ...
    model = ai_settings.get('model', 'gemma4:26b')
    client = ollama.Client(host=host, timeout=180)

    # 모델 크기별 파라미터 조정
    _is_small = 'e2b' in model or 'e4b' in model or '4b' in model or '7b' in model
    NUM_CTX    = 16384 if _is_small else 32768
    CHUNK_TEXT = 20000 if _is_small else 60000
    CHUNK_SIZE = 20    if _is_small else 50
    MAX_WORKERS = 1    if _is_small else 3

    log(f'[AI] Ollama Gemma 연결 중... (host={host}, model={model}, num_ctx={NUM_CTX})')

    log('[AI] PDF 텍스트 추출 중...')
    all_pages = extract_pdf_text_and_tables(pdf_path, log=log)
    log(f'[AI] {len(all_pages)}페이지 추출 완료')

    # ── A: 레지스터 테이블 페이지 필터링 ────────────────────────────────────
    pages = [p for p in all_pages if _is_register_table_page(p)]
    if not pages:
        log('[AI] 레지스터 테이블 페이지 감지 실패 → 전체 페이지 사용', 'warn')
        pages = all_pages
    else:
        log(f'[AI] 레지스터 테이블 페이지 {len(pages)}/{len(all_pages)}페이지 필터링')
    ...
    # ── B: Rule-based 분류 (Pass2 AI 재투입 대체) ──────────────────────────
    final_registers = _classify_pass1_with_rules(deduped, log=log)
    log(f'[RuleClassify] 최종 {len(final_registers)}개 레지스터 확정')
