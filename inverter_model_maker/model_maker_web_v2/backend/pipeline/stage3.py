...
def _gen_register_map(regs_by_cat: dict, mppt: int, total_strings: int,
    ...
    # 우선순위:
    #   1) h01_manual_mapping (Stage2 H01_MAPPING 시트 col D — Stage1 canonical
    #      h01_match 결과 + 사용자 UI 수정 반영)
    #   2) reg.h01_field (Stage1 fuzzy 매칭 — 보조 fallback)
    _h01_to_regname: dict = {}

    def _normalize_regname(_nm: str) -> str:
        if not _nm:
            return ''
    ...
    # 1) h01_manual_mapping 우선 적용 — H01_MAPPING 시트의 사용자 확정 매핑
    if h01_manual_mapping:
        for _h01, _raw_nm in h01_manual_mapping.items():
            _h01 = (_h01 or '').strip()
            _raw_nm = (_raw_nm or '').strip()
            if not _h01 or not _raw_nm:
                continue
            _nm = _normalize_regname(to_upper_snake(_raw_nm))
            if not _nm or _nm not in all_defined:
                continue
            _h01_to_regname[_h01] = _nm

    # 2) reg.h01_field — 누락된 h01_field 에 대해서만 보조로 사용
    for _cat in ('MONITORING', 'STATUS', 'ALARM', 'INFO'):
        for _reg in regs_by_cat.get(_cat, []):
            _h01 = (getattr(_reg, 'h01_field', '') or '').strip()
            if not _h01 or _h01 in _h01_to_regname:
                continue
            _nm = _normalize_regname(to_upper_snake(_reg.definition or ''))
            if not _nm or _nm not in all_defined:
                continue
            _h01_to_regname[_h01] = _nm
